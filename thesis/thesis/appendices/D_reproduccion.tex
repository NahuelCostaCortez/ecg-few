...
La infraestructura VLM/ICL queda preparada como segundo brazo de la comparación. La evaluación final debe lanzarse desde la raíz del repositorio con una única celda de comandos. El bloque valida primero el plan sin inferencia, ejecuta después el simulador QRS/ST, repite el proceso sobre Brugada-HUCA real usando el simulador como banco de demostraciones morfológicas y añade una rama clínica directa en la que el VLM recibe \(K\) pacientes HUCA reales como contexto. En esta rama clínica, cada demostración usa una única derivación fija por paciente, por defecto V2, y para \(K \geq 2\) se fuerzan ejemplos normales y Brugada siempre que existan en el fold. Esto garantiza que Gemma y MedGemma vean exactamente la misma imagen por paciente.
...
\begin{itemize}
  ...
  \item resumen clínico real-context con métricas por modelo, condición y \(K\);
  ...
  \item predicciones por muestra en los árboles de salida morfológico y clínico, separadas por modelo, tarea, condición, \(K\) y semilla;
