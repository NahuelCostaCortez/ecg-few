\chapter{Resultados de la transición al dominio real}
\label{chap:comparacion_real}

El capítulo anterior presentó el primer escalón de la evaluación, compuesto únicamente por imágenes sintéticas del simulador QRS/ST. En ese entorno, la etiqueta está definida por reglas morfológicas y la tarea permite comprobar si los modelos recuperan una señal visual controlada. Este capítulo avanza hacia el dominio clínico de forma gradual.

La lectura se ordena de lo más sintético a lo más real. Primero se describe la cohorte cedida por el Hospital Universitario Central de Asturias, ya que es el destino común de las pruebas clínicas. Después se presentan dos controles de transferencia sintético-real: adaptación de dominio de la CNN usando los datos reales sin etiquetas morfológicas e ICL de Gemma~4 con contexto procedente del simulador. Por último se presenta la comparación principal sobre los ECG reales, formada por la CNN entrenada con el simulador y evaluada sobre la cohorte clínica, y Gemma~4 con demostraciones tomadas de esa misma cohorte.

\section{Datos cedidos por el HUCA y etiqueta clínica}

El bloque real usa ECG cedidos por el Hospital Universitario Central de Asturias dentro del recurso Brugada HUCA \cite{costacortez2026brugada}. A partir de 363 pacientes, el procesado excluye 46 registros con patrón basal limítrofe y conserva un conjunto final de 317 pacientes, formado por 116 casos Brugada confirmados y 201 controles. Para cada paciente se generan imágenes de V1, V2 y V3.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Datos cedidos por el HUCA utilizados en la evaluación real.}
  \label{tab:huca_datos_finales}
\end{table}

La preparación selecciona las derivaciones precordiales derechas, detecta un latido central y recorta una ventana de 300 ms antes de R y 600 ms después de R. Cada imagen queda asociada a su paciente, derivación, etiqueta clínica y pliegue de evaluación.
...
La etiqueta disponible no equivale a las tres etiquetas morfológicas utilizadas en el simulador. Los datos cedidos por el HUCA no incorporan anotaciones independientes para RBBB, elevación ST e inversión de T por imagen. La tarea real se formula por tanto como caso Brugada confirmado frente a control, y sus conclusiones se limitan a priorización para revisión especializada.

\section{Controles de transferencia sintético-real}

Antes de comparar los métodos principales sobre los ECG reales, se evalúa si la información sintética puede reducir por sí sola la brecha de dominio. Esta parte incluye dos controles complementarios. El primero mantiene la CNN como modelo supervisado y añade adaptación de dominio usando los datos reales sin etiquetas morfológicas. El segundo mantiene Gemma~4 sin ajuste de pesos y le proporciona demostraciones del simulador antes de consultar imágenes reales.

\subsection{Adaptación de dominio en la CNN}

En estos experimentos, el simulador sigue siendo el origen etiquetado. Los datos cedidos por el HUCA se usan solo como dominio objetivo no etiquetado. La pérdida supervisada nunca utiliza etiquetas morfológicas procedentes de la cohorte real. Esta restricción conserva la validez metodológica del experimento y evita convertir \texttt{clinical\_brugada} en una etiqueta morfológica que no existe.
...
\begin{table}[H]
  ...
  \begin{tabular}{llrrrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Resultados de adaptación de dominio de la CNN usando los datos cedidos por el HUCA como dominio objetivo no etiquetado.}
...
\subsection{ICL con contexto del simulador}

Como análisis complementario de transferencia, se usaron demostraciones procedentes del simulador y consultas sobre la cohorte clínica cedida por el HUCA. Esta condición no constituye la comparación clínica principal. Su objetivo es medir si unas demostraciones morfológicas sintéticas bastan para orientar al VLM cuando cambia el dominio de la imagen y la referencia de evaluación pasa a ser clínica.
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.94\textwidth}{p{3.7cm}rrrrr}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Puntos principales de Gemma~4 con contexto sintético y consulta sobre los datos reales cedidos por el HUCA. Las cifras son medias sobre tres semillas.}
  \label{tab:vlm_huca_synthetic_context_key}
\end{table}

\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.98\textwidth]{results/transfer_controls_synthetic_to_real_summary.png}
  \caption{Controles de transferencia sintético-real. La parte izquierda resume la adaptación de dominio de la CNN y la parte derecha resume Gemma~4 con demostraciones sintéticas antes de consultar imágenes reales.}
  \label{fig:transfer_controls_summary}
\end{figure}

\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.48\textwidth]{results/vlm_huca_synthetic_context_balanced_k32_confusion_matrix.png}
  \caption{Matriz de confusión agregada de Gemma~4 con contexto sintético balanceado, \(K=32\), y consulta sobre los datos reales cedidos por el HUCA.}
  \label{fig:vlm_huca_synthetic_context_confusion}
\end{figure}

Los dos controles apuntan a la misma lectura. La información sintética ayuda a estudiar una regla visual controlada, pero no basta por sí sola para trasladar el criterio al dominio clínico. En la CNN, la alineación de representaciones produce una mejora limitada. En el VLM, las demostraciones sintéticas apenas modifican el régimen de predicción.

\section{Evaluación directa con los datos cedidos por el HUCA}
...
\begin{table}[H]
  ...
  \begin{tabular}{rrrrrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Resultados CNN ResNet18 entrenada con simulador y evaluada con los datos cedidos por el HUCA. Las cifras son medias sobre tres semillas.}
...
\begin{figure}[H]
  ...
  \caption{Exactitud equilibrada de la CNN base evaluada con los datos cedidos por el HUCA según \(K\). Los puntos muestran la media de tres semillas y las barras la desviación estándar.}
  \label{fig:cnn_huca_ba}
\end{figure}

La matriz agregada para \(K=16\), con 229 verdaderos positivos, 226 verdaderos negativos, 377 falsos positivos y 119 falsos negativos, evidencia el problema central. La CNN recupera una parte importante de los positivos, pero marca demasiados controles.

\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.58\textwidth]{results/cnn_huca_k16_confusion_matrix.png}
  \caption{Matriz de confusión agregada de la CNN evaluada con los datos cedidos por el HUCA para \(K=16\), acumulando las tres semillas.}
  \label{fig:cnn_huca_confusion}
\end{figure}

\subsection{Gemma 4 con contexto real}

El VLM se comporta de forma distinta a la CNN. En zero-shot, Gemma~4 tiende a una decisión casi monoclase y clasifica la práctica totalidad de las imágenes como controles, por lo que conserva una especificidad muy alta, 0,993, pero deja la sensibilidad en 0,023. Este comportamiento no debe interpretarse como evidencia de ausencia de señal en el ECG, sino como una limitación del modelo para fijar la frontera de decisión clínica sin ejemplos locales.

Al añadir demostraciones tomadas de la propia cohorte cedida por el HUCA, el régimen de predicción cambia. El modelo empieza a aceptar con mucha más frecuencia la clase positiva y la sensibilidad aumenta de forma marcada. Con \(K=16\), el ICL normal alcanza BA 0,522, sensibilidad 0,690 y especificidad 0,355, mientras que el ICL balanceado mantiene una BA muy parecida, 0,522, pero sube la sensibilidad a 0,713 y baja la especificidad a 0,332. Por tanto, el contexto no resuelve el problema clínico, pero sí resulta informativo. Unos pocos ejemplos modifican de forma consistente el criterio de decisión frente al colapso conservador del zero-shot.

En las tablas completas, la denominación V2 real identifica este bloque de evaluación con Gemma~4 sobre la imagen de la derivación V2 de cada paciente. Se informa así porque el experimento VLM sobre datos reales utiliza V2 como derivación de consulta principal, manteniendo la etiqueta clínica por paciente.
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.94\textwidth}{p{3.7cm}rrrrr}
    ...
    ICL normal & 16 & 0,522 & 0,491 & 0,690 & 0,355 \\
    ICL balanceado & 16 & 0,522 & 0,496 & 0,713 & 0,332 \\
    ICL balanceado & 32 & 0,521 & 0,484 & 0,658 & 0,385 \\
    \bottomrule
  \end{tabularx}
  \caption{Puntos principales de Gemma~4 con contexto real tomado de los datos cedidos por el HUCA. Las cifras son medias sobre tres semillas.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \safeincludegraphics[width=\textwidth]{results/vlm_huca_v2_ba_by_k.png}
    \caption{Exactitud equilibrada.}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
    \safeincludegraphics[width=\textwidth]{results/vlm_huca_v2_f1_by_k.png}
    \caption{F1.}
  \end{subfigure}
  \caption{Curvas VLM/ICL de Gemma~4 con consulta V2 y contexto real tomado de los datos cedidos por el HUCA. Los puntos muestran la media de tres semillas y las barras la desviación estándar.}
  \label{fig:vlm_huca_curves}
\end{figure}

Las matrices de confusión muestran el cambio de régimen al introducir ejemplos. Gemma~4 pasa de un zero-shot conservador, prácticamente colapsado hacia la clase negativa, a una configuración que detecta más casos, pero también desplaza numerosos controles hacia la clase positiva.

\begin{figure}[H]
  \centering
  \begin{subfigure}{0.42\textwidth}
    \centering
    \safeincludegraphics[width=\textwidth]{results/vlm_huca_v2_normal_k16_confusion_matrix.png}
    \caption{ICL normal, \(K=16\).}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.42\textwidth}
    \centering
    \safeincludegraphics[width=\textwidth]{results/vlm_huca_v2_balanced_k16_confusion_matrix.png}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{4.2cm}p{1.4cm}rrrr}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Puntos relevantes sobre los ECG reales cedidos por el HUCA.}
...
\section{Brecha integrada entre simulador y dominio clínico}

La brecha de dominio afecta a los dos enfoques. En la CNN, la exactitud equilibrada crece en el simulador hasta 0,848 con \(K=32\), mientras que en los ECG reales se mantiene alrededor de 0,50 antes de la adaptación. En el VLM ocurre una compresión similar. El ICL normal alcanza 0,750 en el simulador con \(K=16\), pero baja a 0,522 con contexto real y se queda en torno a 0,500 cuando el contexto procede del simulador.
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \caption{F1.}
  \end{subfigure}
  \caption{Brecha sintético-real de la CNN base. Las líneas continuas corresponden al simulador y las discontinuas a los datos cedidos por el HUCA.}
...
\begin{figure}[H]
  ...
  \caption{Brecha sintético-real de Gemma~4 para exactitud equilibrada. Se comparan simulador, contexto real de los datos cedidos por el HUCA y contexto sintético del simulador.}
...
\begin{figure}[H]
  ...
  \caption{Brecha sintético-real de Gemma~4 para F1. Se comparan simulador, contexto real de los datos cedidos por el HUCA y contexto sintético del simulador.}
  \label{fig:domain_gap_icl_context_f1}
\end{figure}

Las gráficas muestran que el simulador separa mejor la tarea para ambos paradigmas, pero esa ventaja no se conserva cuando la etiqueta pasa a ser clínica. Las causas plausibles incluyen ruido y variabilidad de adquisición, diferencias de escala, presencia de fenotipos diversos, etiqueta clínica no reducible a una sola ventana de ECG y ausencia de anotaciones morfológicas finas. El resultado no invalida el simulador. Delimita su función como banco controlado y muestra que el problema real exige más información que una imagen aislada.
