\chapter{Configuración experimental}
\label{app:configuracion}

Este apéndice recoge los parámetros principales de la configuración experimental. Se incluyen los valores necesarios para interpretar los resultados y comparar los enfoques CNN y VLM/ICL.

\section{Conjunto sintético QRS/ST}

\begin{table}[H]
  \centering
  \small
  \begin{tabularx}{0.94\textwidth}{p{4.4cm}X}
    \toprule
    Propiedad & Valor \\
    \midrule
    Fuente & Simulador QRS/ST descrito en el capítulo~\ref{chap:simulador} \\
    Pacientes & 100 \\
    Imágenes & 300 \\
    Familias fuente & NORMAL, RBBB, ST\_ELEVATION, T\_WAVE\_INVERSION, COMBINED\_QRS\_ST \\
    Pacientes por familia & 20 \\
    Derivaciones renderizadas & V1, V2 y V3 \\
    Frecuencia de muestreo & 500 Hz \\
    Duración del beat & 900 ms \\
    Resolución de guardado & 130 ppp \\
    Positivos derivados & 20 \\
    Negativos o incompletos & 80 \\
    Valores de \(K\) & \(K=\{2,4,8,16,32\}\) \\
    Semillas & 42, 123 y 2026 \\
    \bottomrule
  \end{tabularx}
  \caption{Configuración del conjunto sintético final.}
  \label{tab:config_sintetico_final}
\end{table}

La etiqueta positiva se obtiene cuando RBBB, elevación ST e inversión T están presentes simultáneamente. La agregación de paciente promedia las probabilidades de las derivaciones y después aplica el umbral seleccionado.

\section{Datos cedidos por el HUCA}

\begin{table}[H]
  \centering
  \small
  \begin{tabularx}{0.94\textwidth}{p{4.4cm}X}
    \toprule
    Propiedad & Valor \\
    \midrule
    Fuente & Hospital Universitario Central de Asturias (HUCA), recurso Brugada HUCA 1.0.0 \\
    Pacientes brutos & 363 \\
    Pacientes excluidos & 46 por patrón basal limítrofe \\
    Pacientes incluidos & 317 \\
    Imágenes incluidas & 951 \\
    Casos confirmados & 116 \\
    Controles & 201 \\
    Derivaciones & V1, V2 y V3 \\
    Ventana & 300 ms antes de R y 600 ms después de R \\
    Frecuencia de origen & 100 Hz \\
    Resolución de guardado & 130 ppp \\
    Separación & Leave-one-out por paciente \\
    \bottomrule
  \end{tabularx}
  \caption{Configuración del conjunto real procesado.}
  \label{tab:config_huca_final}
\end{table}

Los datos cedidos por el HUCA no contienen anotaciones independientes para las tres etiquetas morfológicas del simulador. La tarea real utiliza la etiqueta clínica \texttt{clinical\_brugada}, recogida a nivel de paciente mediante revisión especializada.

\section{Configuración CNN}

\begin{table}[H]
  \centering
  \small
  \begin{tabularx}{0.94\textwidth}{p{4.4cm}X}
    \toprule
    Propiedad & Valor \\
    \midrule
    Arquitectura & ResNet18 \\
    Pesos iniciales & Inicialización estándar de ResNet18 \\
    Entrada & RGB, 224 por 224 \\
    Salida & Tres hallazgos morfológicos \\
    Pérdida & Entropía cruzada binaria multietiqueta \\
    Optimizador & AdamW \\
    Tasa de aprendizaje & \(3\times10^{-4}\) \\
    Lote & 32 \\
    Épocas máximas & 20 \\
    Estrategia de umbral & Selección en validación mediante exactitud equilibrada \\
    Métrica principal & Exactitud equilibrada por paciente \\
    Grad CAM & 12 paneles en sintético y 6 en datos reales por ejecución agregada \\
    \bottomrule
  \end{tabularx}
  \caption{Hiperparámetros de la CNN final.}
  \label{tab:config_cnn_appendix}
\end{table}

\section{Configuración de adaptación de dominio}

\begin{table}[H]
  \centering
  \small
  \begin{tabularx}{0.94\textwidth}{p{4.4cm}X}
    \toprule
    Propiedad & Valor \\
    \midrule
    Origen etiquetado & Conjunto sintético QRS/ST \\
    Destino no etiquetado & Datos cedidos por el HUCA \\
    Métodos & SSL, CORAL, MMD y DANN \cite{chen2020simclr,sun2016deepcoral,long2015dan,ganin2016} \\
    Valores de \(K\) & \(K=16\) y \(K=32\) \\
    Semillas & 42, 123 y 2026 \\
    Peso de adaptación & 0,1 \\
    Escalas MMD & 0,5, 1,0, 2,0 y 4,0 \\
    SSL & SimCLR durante 3 épocas \\
    Política de destino & Transductiva compartida, sin etiquetas destino en la pérdida supervisada \\
    \bottomrule
  \end{tabularx}
  \caption{Configuración de los experimentos de adaptación de dominio.}
  \label{tab:config_domain_appendix}
\end{table}

\section{Configuración VLM/ICL}

El enfoque VLM/ICL usa los mismos pacientes reservados, valores de \(K\) y métricas por paciente que la CNN. La evaluación final corresponde a Gemma~4, ejecutado con temperatura 0 y salida estructurada.

\begin{table}[H]
  \centering
  \small
  \begin{tabularx}{0.94\textwidth}{p{4.4cm}X}
    \toprule
    Elemento & Valor \\
    \midrule
    Modelo informado & Gemma~4 \\
    Entrada & Imagen ECG y demostraciones imagen-etiqueta \\
    Valores de \(K\) & \(K=\{0,2,4,8,16,32\}\) \\
    Condiciones informadas & Zero-shot, ICL normal e ICL balanceado \\
    Contextos sobre datos reales & Contexto tomado de la cohorte cedida por el HUCA y contexto sintético del simulador \\
    Salida morfológica & RBBB, elevación ST e inversión de T \\
    Salida clínica & Caso Brugada confirmado frente a control \\
    Métrica principal & Exactitud equilibrada por paciente \\
    \bottomrule
  \end{tabularx}
  \caption{Configuración resumida del enfoque VLM/ICL.}
  \label{tab:config_vlm_appendix}
\end{table}

El capítulo~\ref{chap:vlm} describe este enfoque y los capítulos de resultados desarrollan su comparación con la CNN.
