...
\section{Dominio sintético}
\label{chap:comparacion_simulador}

El simulador QRS/ST actúa como dominio sintético del estudio y permite evaluar una regla morfológica explícita antes de pasar a los ECG reales. En esta tarea, un caso se considera positivo solo cuando aparecen a la vez tres hallazgos visuales: morfología compatible con RBBB, elevación del segmento ST e inversión de la onda T. Por ejemplo, una imagen con elevación ST e inversión de T, pero sin morfología compatible con RBBB, se mantiene como negativa. Este apartado no pretende demostrar utilidad clínica directa, sino comprobar si la CNN y el VLM con ICL identifican una combinación visual definida cuando el ECG se representa como imagen.
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.94\textwidth}{p{3.6cm}rrrrr}
    ...
    ICL estándar & 16 & 0,750 & 0,529 & 0,817 & 0,683 \\
...
El ICL estándar alcanza su mejor punto en \(K=16\), con 0,750 de exactitud equilibrada. El ICL balanceado obtiene el máximo VLM con 0,754 en \(K=8\). La diferencia entre ambos no está solo en la métrica principal: el contexto balanceado aumenta mucho la sensibilidad, pero paga ese aumento con menor especificidad.

\begin{figure}[H]
  \centering
  \begin{subfigure}{0.42\textwidth}
    \centering
    \safeincludegraphics[width=\textwidth]{results/vlm_simulator_qrs_estandar_k16_confusion_matrix.png}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \caption{F1.}
  \end{subfigure}
  \caption{Comparación gráfica entre CNN, ICL estándar e ICL balanceado en el simulador QRS/ST. Las curvas usan las mismas métricas y los mismos valores de \(K\) para los tres enfoques.}
...
El resultado del dominio sintético es favorable para ambos enfoques, aunque con ventaja cuantitativa para la CNN. No debe trasladarse directamente a clínica. El simulador es limpio, sus etiquetas están delimitadas por reglas y sus imágenes no incluyen toda la variabilidad de un ECG real. Su valor está en mostrar que la formulación visual funciona y en ofrecer un punto de comparación definido para interpretar después la adaptación al dominio clínico.

\input{chapters/10_resultados_reales}
