...
La disponibilidad de datos sintéticos permite estudiar una pregunta metodológica sin esperar a reunir una colección clínica extensa. En este trabajo el simulador actúa como un laboratorio sintético. Cada beat se construye a partir de componentes interpretables, se valida mediante reglas explícitas y se transforma en una imagen que puede entregarse tanto a una CNN como a un modelo de visión y lenguaje.

El objetivo no es reproducir toda la fisiología cardíaca. Un simulador clínicamente exhaustivo tendría que modelar varias derivaciones, cambios de frecuencia, interacción entre ciclos, conducción auricular y ventricular, ruido de adquisición, variación anatómica y efectos del dispositivo. El sistema implementado reduce el problema a un único beat de apariencia precordial derecha. Esta simplificación permite parametrizar tres hallazgos visuales que intervienen en la motivación del proyecto.
...
La contrapartida es una brecha inevitable entre el dominio sintético y el real. El simulador puede demostrar que una tarea definida por reglas es aprendible. No puede demostrar que el mismo rendimiento se mantenga en un ECG clínico. Esa distinción se conserva en toda la memoria.
...
El signo de \(a_i\) define la dirección de la deflexión. El valor absoluto de \(a_i\) determina su magnitud. El parámetro \(\mu_i\) desplaza el componente sobre el eje temporal y \(\sigma_i\) modifica su extensión. La suma permite formar transiciones suaves sin introducir discontinuidades.
...
Modificar una regla puede tener efectos indirectos. Mover T1 cambia el resultado del validador de inversión y también puede alterar la media tardía del ST. Cada cambio debe acompañarse, por tanto, de una nueva generación de ejemplos, un análisis de frecuencias y una comprobación de aceptación.
...
El renderizado conserva títulos de ejes y valores numéricos. Estos elementos aportan información de escala, pero también pueden transformarse en atajos si los límites cambian entre muestras. Por ese motivo, los límites horizontal y vertical se fijan para todas las imágenes del experimento final. La inspección visual verifica que ninguna familia posee un encuadre, margen o patrón de marcas propio.
...
La generación del conjunto sintético parte de las familias definidas y asigna a cada muestra una semilla propia. Una vez aceptadas las señales, se crean las particiones de entrenamiento, validación y prueba y se guardan las imágenes junto con el manifiesto correspondiente.

Cada muestra queda asociada a su imagen, partición, familia generadora, semilla, subtipo cuando procede, número de intento e índices internos, junto con las etiquetas binarias correspondientes. Además, se guardan ficheros auxiliares con el esquema de etiquetas y resúmenes de frecuencia por partición, familia y combinación de clases. De este modo, el entrenamiento puede realizarse directamente desde los manifiestos CSV, sin depender de la organización física de las carpetas, y los ficheros JSON quedan como apoyo para revisar el proceso o elaborar resultados.
...
El conjunto generado constituye la base de la evaluación sintética posterior. La comprobación de integración confirma que la generación de señales, la aceptación por validadores, la partición por paciente y la escritura de manifiestos producen un artefacto experimental reutilizable. La presencia de varias combinaciones de etiquetas permite estudiar la evolución del rendimiento al variar \(K\) sin reducir la tarea a una separación trivial entre normalidad y un único patrón patológico.

La revisión de los ejemplos generados confirma que la familia de origen y las etiquetas finales cumplen funciones distintas. Una familia modifica los rangos de muestreo para favorecer una morfología concreta, pero los validadores se aplican después sobre la señal resultante. Por eso una señal orientada a elevación del ST puede activar también RBBB, y una señal con inversión de T puede conservar una deflexión terminal compatible con otros hallazgos. La tarea experimental queda definida por los rasgos detectados en la señal, no por el nombre de la familia que inició la generación.
...
En la familia Brugada se modela de forma simplificada una elevación del ST con morfología convexa, asociada al patrón de Brugada tipo 1. No representa la heterogeneidad del síndrome, la variación entre derivaciones ni la posibilidad de patrones intermitentes. Su uso se limita a estudiar la composición visual de los tres hallazgos.

Estas limitaciones no invalidan el laboratorio sintético. Delimitan qué afirmaciones puede sostener. El dominio permite comparar eficiencia de ejemplos bajo condiciones sintéticas. La validez clínica dependerá de la evaluación externa descrita en el capítulo de datos reales.
