...
La infraestructura VLM/ICL queda preparada como segundo brazo de la comparación. Antes de lanzar inferencias se valida el plan completo sin cargar modelos ni hacer llamadas de red:
...
La campaña completa contra un servidor vLLM compatible con la API de OpenAI se ejecuta con:

\begin{lstlisting}[language=bash]
VLM_API_BASE=http://servidor:8000/v1 \
VLM_MODELS=google/gemma-4-E4B-it,google/medgemma-4b-it \
scripts/run/run_vlm_loocv.sh
\end{lstlisting}

Por defecto se ejecutan \(K=\{0,2,4,8,16,32\}\), semillas 42, 123 y 2026, y las condiciones \texttt{zero\_shot}, \texttt{normal}, \texttt{balanced}, \texttt{permuted} y \texttt{no\_support\_images}. Los controles usan \texttt{CONTROL\_K\_VALUES=8,16,32}. Para probar la ruta de artefactos sin GPU ni API puede usarse:
...
La comparación CNN frente a VLM se reconstruye, una vez existan inferencias, con:
