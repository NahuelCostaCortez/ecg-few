...
\section{Alcance experimental}

La tabla~\ref{tab:estado_experimentos} distingue los bloques con resultados medidos de los bloques definidos como protocolo. La línea CNN y la comprobación complementaria aportan valores numéricos completos. La línea VLM/ICL aporta especificación de modelos, mensajes, controles, esquema JSON y criterios de reporte.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.6cm}p{2.6cm}X}
    ...
    \textbf{VLM e ICL} & \textbf{Protocolo definido} & Modelos, mensajes, controles, esquema JSON, agregación por paciente y criterios de reporte \\
    Auditoría final & Completa para los bloques con resultados & Predicciones por paciente, matrices y métricas agregadas verificadas; protocolo VLM definido con las mismas unidades de análisis \\
    \bottomrule
  \end{tabularx}
  \caption{Alcance de los bloques experimentales.}
  \label{tab:estado_experimentos}
\end{table}

Esta separación mantiene completo el diseño comparativo sin mezclar resultados medidos y protocolo. Las conclusiones numéricas proceden de la CNN y de la comprobación complementaria; el capítulo~\ref{chap:vlm} fija la estructura de lectura para Gemma 4 y MedGemma.
