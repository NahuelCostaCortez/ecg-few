...
\section{Configuración VLM pendiente}

La línea VLM forma parte del diseño comparativo, pero no de las métricas cerradas. La configuración prevista se mantiene para que la siguiente fase use la misma base experimental que la CNN:

\begin{itemize}
  \item modelos abiertos: Gemma 4 y MedGemma;
  \item servidor local compatible con API de vLLM;
  \item temperatura 0;
  \item salida JSON validada;
  \item presupuestos \(K=\{0,2,4,8,16,32\}\) o el subconjunto que se congele antes de ejecutar;
  \item controles con respuestas permutadas y ejemplos sin imagen.
\end{itemize}

Los resultados VLM deberán añadirse únicamente cuando existan predicciones por muestra, tablas agregadas, matrices de confusión y auditoría equivalente a la de CNN. Hasta entonces, la rama CNN actúa como línea base cerrada y la rama VLM/ICL como comparador central pendiente.
