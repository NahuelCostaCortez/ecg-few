...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.94\textwidth]{results/fig_gaussian_atoms.pdf}
  \caption{Construcción de un beat mediante átomos gaussianos y suma resultante.}
  \label{fig:gaussian_atoms}
\end{figure}
...
Separar rangos y validadores responde a dos responsabilidades distintas. Los rangos hacen probable una morfología. Los validadores determinan si la señal terminada satisface la definición operativa. Si ambos elementos se mezclaran, cualquier cambio visual podría alterar de manera silenciosa el significado de las etiquetas.
...
\midrule
\texttt{NORMAL} & ST próximo a cero, T positiva y R prima mínima & Referencia sin hallazgos operativos \\
...
Una familia que requiere muchos intentos puede seguir generando imágenes correctas, pero revela que gran parte de su espacio paramétrico no satisface la definición operativa. Las tasas de aceptación deben registrarse cuando se cierre la versión final del simulador.
...
\begin{table}[H]
\centering
\caption{Criterios operativos de los validadores sintéticos}
...
\begin{figure}[H]
  \centering
  \safeincludegraphics[width=0.92\textwidth]{results/fig_synthetic_examples.pdf}
...
\small
\begin{tabularx}{\textwidth}{p{3.15cm} p{1.45cm} p{5.15cm} X}
\toprule
...
Imágenes & 300 & Copia del beat central en V1, V2 y V3 & Evaluar el mismo contrato visual que en HUCA \\
...
Esta comprobación no se limita a una prueba de humo: es el conjunto utilizado en la campaña CNN sintética. Su función es demostrar que la ruta de generación, aceptación, partición y escritura es operativa y que el conjunto final conserva variabilidad suficiente para estudiar la curva por \(K\).
